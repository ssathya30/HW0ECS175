...
class Shape
{
    ...
    // Link the VBO to the shader attribute 'a_position'
    ...
    render( gl )
    {
        ...
        // Get the uniform location for "u_color" in your shader program
        const colorUniformLocation = gl.getUniformLocation(this.shader.program, 'u_color');
        
        if (colorUniformLocation !== null) {
            // Create a Float32Array with the color values
            const colorArray = new Float32Array(this.color);
            
            // Set the uniform for "u_color"
            gl.uniform3fv(colorUniformLocation, colorArray);
        }
        
        // Draw the element using the specified draw mode (e.g., gl.TRIANGLES)
...
class Triangle extends Shape
{
    ...
    constructor( gl, shader, position, color, sideLength) 
    {
        ...
        // Define indices to form the triangle
        const indices = [0, 1, 2];
...
class WebGlApp 
{
    ...
    addTriangle( gl, shader, position, sideLength )
    {
        ...
        const color = [1.0, 0.749, 0]; // Aggie Gold color in RGB
